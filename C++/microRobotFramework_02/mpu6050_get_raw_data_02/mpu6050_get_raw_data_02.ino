...
// We use software seiral for raspberry pi -> arduino motor control simulation and debugging  
...
void receiveMotorCommand() {
  if (Serial.available()) {  // Packet size is 5 bytes
    uint8_t packet[5];
    Serial.readBytes(packet, 5);

    // Verify header (first byte should be 0xf5)
    if (packet[0] != 0xf5) return;
